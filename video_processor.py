...
class VideoProcessor:
    def __init__(self, video_path, calibration_file='camera_calibration.json', road_mask_file='road_mask.npy',
        ...
        self.out = cv2.VideoWriter(self.output_path, self.fourcc, self.fps, (self.width * 2, self.height * 2))
    ...
    def process_video(self):
        ...
        while True:
            ...
            try:
                # Detect vehicles
                detections = self.car_detection.detect_cars(ipm_frame, self.ipm_matrix, self.calibration, self.detection_confidence)
                ...
                for track_id, track in tracks.items():
                    ...
                    # Refine speed using optical flow
                    if flow is not None:
                        flow_in_roi = self.flow_estimator.get_flow_in_roi(flow, corners_2d.ravel())
                        if flow_in_roi.size > 0:
                            mean_flow = np.mean(flow_in_roi, axis=0)
                            flow_speed = np.linalg.norm(mean_flow) * self.fps * 3.6  # Convert to km/h
                            speed = (speed + flow_speed) / 2 if speed is not None else flow_speed
                ...
                print(f"Error in processing frame {frame_count}: {str(e)}")
            ...
            flow_vis = self.flow_estimator.visualize_flow(vis_frame, flow)
            ...
            combined_vis = cv2.resize(combined_vis, (self.width * 2, self.height * 2))
